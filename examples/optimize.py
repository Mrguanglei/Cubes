...
from Enval import evaluate_metrics
from util import *
import render
import loss
import imageio
...
#定义学习率  这个函数定义了一个学习率调度器，学习率随着迭代次数的增加而指数下降
if __name__ == "__main__":
    ...
    for it in range(FLAGS.iter):
        ...
        if (it % FLAGS.save_interval == 0 or it == (FLAGS.iter-1)): # save normal image for visualization
            # 进行评估
            metrics = evaluate_metrics(gt_mesh, flexicubes_mesh)
            print(f"Metrics at Iteration {it}: {metrics}")
            # 其他保存代码
            with torch.no_grad():
    ...
    plt.legend()
    plt.savefig(os.path.join(FLAGS.out_dir, 'loss_curve.png'))
    plt.show()